        ...
        <div class="modal-body">
            <div class="expanded-content-frame" style="padding: 1.5rem; border: none;">
                <div class="space-y-4 mb-6">
                    <h4 class="text-md font-medium text-primary">Nuova Transazione</h4>
                    <div class="flex items-center gap-4 w-full expanded-row">
                        <input type="text" id="transaction-description-${client.id}" class="form-control" placeholder="Descrizione (es. Carburante)" value="${amministrazioneState.transactionForm.description}" autocomplete="off">
                        <div class="flex items-center gap-4">
                            <input type="number" id="transaction-amount-${client.id}" step="0.01" class="form-control" placeholder="€" value="${amministrazioneState.transactionForm.amount || ''}" autocomplete="off" style="width: 120px;">
                            <div class="flex items-center space-x-2">
                                <button class="btn btn-danger" onclick="addTransactionInline('${client.id}', 'debit')" title="Addebita"><i data-lucide="circle-minus" style="margin-right: 0;"></i></button>
                                <button class="btn btn-warning" onclick="addTransactionInline('${client.id}', 'credit')" title="Acconto"><i data-lucide="circle-plus" style="margin-right: 0;"></i></button>
                                <button class="btn btn-success" onclick="settleAccountInline('${client.id}')" title="Salda Conto"><i data-lucide="euro" style="margin-right: 0;"></i></button>
                                <button class="btn btn-secondary" onclick="printAccountInline('${client.id}')" title="Stampa"><i data-lucide="printer" style="margin-right: 0;"></i></button>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="space-y-4">
                    <div class="flex justify-between items-center">
                        <h4 class="text-md font-medium text-primary">Estratto Conto</h4>
                        <div class="text-right">
                            <div class="text-sm text-secondary">Saldo Attuale:</div>
                            <span class="font-bold text-md ${app.getBalanceClass(client.balance)}">${app.formatCurrency(client.balance)}</span>
                        </div>
                    </div>
                    ${transactions.length > 0 ?
                        `<div class="table-container" style="max-height: 220px; overflow-y: auto;">
                            <table class="table editable-table">
                                <thead>
                                    <tr>
                                        <th style="width: 20%;">Data</th><th style="width: 50%;">Descrizione</th>
                                        <th style="width: 20%;">Importo</th><th style="width: 10%;">Azioni</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ${transactions.map(tx => `
                                        <tr data-transaction-id="${tx.id}">
                                            <td>
                                                <span class="editable-cell" data-field="date" data-client-id="${client.id}" data-tx-id="${tx.id}">${app.formatDate(tx.date)}</span>
                                                <input type="text" class="form-control edit-input hidden" value="${app.formatToItalianDate(tx.date)}" style="font-size: var(--font-size-small); padding: 0.25rem;" autocomplete="off">
                                            </td>
                                            <td>
                                                <span class="editable-cell" data-field="description" data-client-id="${client.id}" data-tx-id="${tx.id}">${tx.description}</span>
                                                <input type="text" class="form-control edit-input hidden" value="${tx.description}" style="font-size: var(--font-size-small); padding: 0.25rem;" autocomplete="off">
                                            </td>
                                            <td>
                                                <span class="editable-cell font-bold ${tx.amount > 0 ? 'text-success' : 'text-danger'}" data-field="amount" data-client-id="${client.id}" data-tx-id="${tx.id}">${formatTransactionAmount.call(app, tx.amount)}</span>
                                                <input type="number" step="0.01" class="form-control edit-input hidden" value="${tx.amount}" style="font-size: var(--font-size-small); padding: 0.25rem;" autocomplete="off">
                                            </td>
                                            <td class="text-right">
                                                <div class="flex items-center justify-end space-x-1">
                                                    <button class="btn btn-success btn-xs edit-btn" onclick="toggleEditTransaction('${client.id}', '${tx.id}')" title="Modifica"><i data-lucide="edit"></i></button>
                                                    <button class="btn btn-success btn-xs save-btn hidden" onclick="saveEditTransaction('${client.id}', '${tx.id}')" title="Salva"><i data-lucide="check"></i></button>
                                                    <button class="btn btn-danger btn-xs" onclick="deleteTransactionInline('${client.id}', '${tx.id}')" title="Elimina"><i data-lucide="trash-2"></i></button>
                                                </div>
                                            </td>
                                        </tr>
                                    `).join('')}
...
// Inizio funzione handleAdminClick
function handleAdminClick(event) {
    ...
    if (sortBtn) sortAdmin.call(app, sortBtn.dataset.sort);
}
// Fine funzione handleAdminClick

// Inizio funzione handleAdminInput
...
function sortedClients() {
    ...
    let clients = [...this.state.data.clients].map(c => ({
        ...
        lastTransactionDate: Array.isArray(c.transactions) && c.transactions.length > 0 ?
            new Date(c.transactions.reduce((latest, current) => new Date(current.date) > new Date(latest.date) ? current : latest).date) : null
    }));
            ...
            <tr class="hover:bg-secondary">
                ...
                <td class="text-right">
                    <div class="flex items-center justify-end space-x-2">
                        <button class="btn btn-info" onclick="showClientModalById('${client.id}')" title="Gestisci Cliente"><i data-lucide="user-cog"></i></button>
                        <button class="btn btn-danger" onclick="deleteClientById('${client.id}')" title="Elimina cliente"><i data-lucide="trash-2"></i></button>
                    </div>
                </td>
            </tr>
        `).join('');
