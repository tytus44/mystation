...
(function() {
    ...
    const AdminModule = {
        ...
        saveLayout() {
            ...
            localStorage.setItem('Polaris_admin_layout_v2', JSON.stringify(layout));
        },

        restoreLayout() {
            const saved = localStorage.getItem('Polaris_admin_layout_v2');
            if (!saved) return;
            try {
            ...
            return `
                <div id="admin-layout" class="flex flex-col gap-6 animate-fade-in">
                    ...
                    <div id="admin-sections" class="flex flex-col gap-6">
                        <div id="sec-stats" class="group"><div id="admin-stats" class="grid grid-cols-1 sm:grid-cols-3 gap-4 items-start"></div></div>
                        <div id="card-clients" class="bg-white border border-gray-200 rounded-lg shadow-none dark:bg-gray-800 dark:border-gray-700 overflow-hidden draggable-card group">
                            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700 card-header cursor-move section-handle transition-colors"><h3 class="text-xl font-bold text-gray-900 dark:text-white tracking-tight" id="admin-view-title">Gestione Clienti</h3><i data-lucide="grip-horizontal" class="w-5 h-5 text-gray-400"></i></div>
        ...
        renderStats(forceRender = false) {
            ...
            if (!container) return;
            if (!forceRender && document.getElementById('val-stat-clients')) { document.getElementById('val-stat-clients').textContent = clients.length; document.getElementById('val-stat-credit').textContent = App.formatCurrency(totalCredit).replace('€',''); document.getElementById('val-stat-debit').textContent = App.formatCurrency(Math.abs(totalDebit)).replace('€',''); } 
            else { container.innerHTML = `${this.renderStatCard('stat-clients', 'Clienti Attivi', 'val-stat-clients', clients.length, 'bg-blue-600', 'users')}${this.renderStatCard('stat-credit', 'Totale Credito', 'val-stat-credit', App.formatCurrency(totalCredit).replace('€',''), 'bg-green-600', 'trending-up')}${this.renderStatCard('stat-debit', 'Totale Debito', 'val-stat-debit', App.formatCurrency(Math.abs(totalDebit)).replace('€',''), 'bg-red-600', 'trending-down')}`; lucide.createIcons(); }
        },
        renderStatCard(id, title, valId, value, iconBg, iconName) {
            return `<div id="${id}" class="bg-white border border-gray-200 rounded-lg shadow-none dark:bg-gray-800 dark:border-gray-700 draggable-card cursor-move overflow-hidden transition-colors"><div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700 card-header"><h3 class="text-sm font-semibold text-gray-600 dark:text-gray-300">${title}</h3><div class="flex items-center justify-center w-10 h-10 rounded-full ${iconBg} text-white shadow-sm"><i data-lucide="${iconName}" class="w-5 h-5"></i></div></div><div class="p-6"><div class="text-2xl font-bold text-gray-900 dark:text-white tracking-tight" id="${valId}">${value}</div></div></div>`;
        },
        updateViewButtons() {
            ...
            const titleEl = document.getElementById('admin-view-title'); if(titleEl) titleEl.textContent = this.localState.currentView === 'list' ? 'Elenco Clienti' : 'Griglia Clienti';
        },
        renderTable() {
            const clients = this.getFilteredClients(); const content = document.getElementById('admin-content-area'); if (!content) return; if (!clients.length) { content.innerHTML = '<div class="text-center text-gray-500 dark:text-gray-400 py-8">Nessun cliente trovato.</div>'; return; }
            content.innerHTML = `<div class="overflow-x-auto"><table class="w-full text-sm text-left text-gray-500 dark:text-gray-400"><thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400"><tr><th class="px-4 py-3">Nome Cliente</th><th class="px-4 py-3">Saldo Attuale</th><th class="px-4 py-3">Ultima Operazione</th><th class="px-4 py-3 text-right">Azioni</th></tr></thead><tbody class="divide-y divide-gray-200 dark:divide-gray-700">${clients.map(c => { const balClass = c.balance > 0 ? 'text-green-600 dark:text-green-400 font-bold' : (c.balance < 0 ? 'text-red-600 dark:text-red-400 font-bold' : 'text-gray-900 dark:text-white'); const lastTx = this.getLastTransaction(c); return `<tr class="border-b dark:border-gray-700 transition-colors"><td class="px-4 py-3 font-medium text-gray-900 dark:text-white">${c.name}</td><td class="px-4 py-3 ${balClass}">${App.formatCurrency(c.balance)}</td><td class="px-4 py-3">${lastTx ? App.formatDate(lastTx.date) : '-'}</td><td class="px-4 py-3 text-right"><button class="font-medium text-primary-600 dark:text-primary-400 btn-manage-client" data-id="${c.id}">Gestisci</button></td></tr>`; }).join('')}</tbody></table></div>`;
            this.attachDynamicListeners();
        },
        renderGrid() {
            const clients = this.getFilteredClients(); const content = document.getElementById('admin-content-area'); if (!content) return; if (!clients.length) { content.innerHTML = '<div class="text-center text-gray-500 dark:text-gray-400 py-8">Nessun cliente trovato.</div>'; return; }
            content.innerHTML = `<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 items-start">${clients.map(c => { const balClass = c.balance > 0 ? 'text-green-600 dark:text-green-400' : (c.balance < 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-white'); const lastTx = this.getLastTransaction(c); return `<div class="bg-white border border-gray-200 rounded-lg shadow-none dark:bg-gray-800 dark:border-gray-700 flex flex-col cursor-pointer transition-colors btn-manage-client overflow-hidden" data-id="${c.id}"><div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-700/30"><h3 class="text-base font-bold text-gray-900 dark:text-white truncate" title="${c.name}">${c.name}</h3><i data-lucide="user" class="w-5 h-5 text-gray-400"></i></div><div class="p-5 space-y-4"><div class="flex justify-between items-center"><span class="text-sm text-gray-500 dark:text-gray-400">Saldo:</span><span class="text-xl font-bold ${balClass}">${App.formatCurrency(c.balance)}</span></div><div class="text-xs text-gray-500 dark:text-gray-400 pt-3 border-t border-gray-100 dark:border-gray-700 flex items-center"><i data-lucide="calendar" class="w-3 h-3 mr-2"></i> ${lastTx ? 'Ult. op: ' + App.formatDate(lastTx.date) : 'Nessuna operazione'}</div></div></div>`; }).join('')}</div>`;
            lucide.createIcons(); this.attachDynamicListeners();
        },
        ...
        openClientModal(id=null) {
            if(!id) {
                ...
                App.showModal('Nuovo Cliente', form, '<button id="btn-save-client" class="text-white bg-primary-600 font-semibold rounded-md text-sm px-5 py-2.5 transition-none shadow-sm">Salva</button>', 'max-w-md');
                document.getElementById('btn-save-client').onclick = () => this.saveNewClient(); setTimeout(() => document.getElementById('client-name-input')?.focus(), 50);
            } else { this.localState.editingClientId = id; this.renderClientModal(id); }
        },
        ...
        deleteClient(id) { const c = App.state.data.clients.find(x=>x.id===id); this.showDeleteModal('Eliminare cliente?', `Stai per eliminare <b>${c.name}</b>. L'azione è irreversibile.`, () => { App.state.data.clients = App.state.data.clients.filter(x=>x.id!==id); App.saveToStorage(); App.closeModal(); this.localState.editingClientId = null; this.updateView(); }); },
        deleteTransaction(client, txId) { this.showDeleteModal('Eliminare transazione?', 'Questa azione è irreversibile.', () => { client.balance -= client.transactions.find(t=>t.id===txId).amount; client.transactions = client.transactions.filter(t=>t.id!==txId); App.saveToStorage(); this.renderClientModal(client.id); this.updateView(); }); },
        settleAccount(client) { if(client.balance === 0) return alert('Il saldo è già zero.'); this.showDeleteModal('Saldare conto?', `Vuoi azzerare il saldo di <b>${client.name}</b>?`, () => { client.balance = 0; client.transactions = []; App.saveToStorage(); this.renderClientModal(client.id); this.updateView(); }); },
        
        // --- MODIFICA PRINT: INSERITO MONTSERRAT ---
        printStatement(client) { const w = window.open('', '_blank'); w.document.write(`<html><head><title>Estratto Conto - ${client.name}</title><link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@300;400;600&display=swap" rel="stylesheet"><style>body{font-family:'Montserrat',sans-serif;padding:20px;font-weight:300}table{width:100%;border-collapse:collapse;margin-top:20px}th,td{border:1px solid #ddd;padding:8px;text-align:left}th{background-color:#f2f2f2;font-weight:600}.text-right{text-align:right}.text-red{color:#dc2626}.text-green{color:#16a34a}h2{font-weight:600}</style></head><body><h2>Estratto Conto: ${client.name}</h2><p>Data: ${new Date().toLocaleDateString('it-IT')}</p><table><thead><tr><th>Data</th><th>Descrizione</th><th class="text-right">Importo</th></tr></thead><tbody>${client.transactions.map(t => `<tr><td>${new Date(t.date).toLocaleDateString('it-IT')}</td><td>${t.description}</td><td class="text-right ${t.amount<0?'text-red':'text-green'}">${new Intl.NumberFormat('it-IT',{style:'currency',currency:'EUR'}).format(t.amount)}</td></tr>`).join('')}</tbody><tfoot><tr><th colspan="2" class="text-right">Saldo Finale:</th><th class="text-right">${new Intl.NumberFormat('it-IT',{style:'currency',currency:'EUR'}).format(client.balance)}</th></tr></tfoot></table></body></html>`); w.document.close(); w.print(); },
        
        printList() { const clients = this.getFilteredClients(); const w = window.open('', '_blank'); let rows = ''; for (let i = 0; i < clients.length; i += 2) { const c1 = clients[i]; const c2 = clients[i+1]; const renderCell = (c) => { if (!c) return '<td></td><td></td>'; const colorClass = c.balance > 0 ? 'text-green' : (c.balance < 0 ? 'text-red' : ''); return `<td class="name-cell">${c.name}</td><td class="text-right ${colorClass}">${App.formatCurrency(c.balance)}</td>`; }; rows += `<tr>${renderCell(c1)}<td class="separator"></td>${renderCell(c2)}</tr>`; } w.document.write(`<html><head><title>Lista Clienti</title><link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@300;400;600&display=swap" rel="stylesheet"><style>body{font-family:'Montserrat',sans-serif;padding:10px;font-size:12px;font-weight:300}h2{margin-bottom:5px;text-align:center;font-size:16px;font-weight:600}p{text-align:center;margin-top:0;color:#666;font-size:10px}table{width:100%;border-collapse:collapse;margin-top:10px}th{background-color:#f2f2f2;padding:6px;text-align:left;border-bottom:2px solid #ccc;font-size:11px;font-weight:600}td{border-bottom:1px solid #eee;padding:6px;vertical-align:top}.text-right{text-align:right;white-space:nowrap}.text-red{color:#dc2626}.text-green{color:#16a34a}.name-cell{font-weight:600;color:#333;width:35%}.separator{width:2%;border:none;background:#fff}@media print{@page{margin:0.5cm}}</style></head><body><h2>Lista Clienti</h2><p>Generato il: ${new Date().toLocaleDateString('it-IT')} alle ${new Date().toLocaleTimeString('it-IT')}</p><table><thead><tr><th>Cliente</th><th class="text-right">Saldo</th><th class="separator"></th><th>Cliente</th><th class="text-right">Saldo</th></tr></thead><tbody>${rows}</tbody></table><script>window.onload=function(){window.print();window.close();}</script></body></html>`); w.document.close(); },
