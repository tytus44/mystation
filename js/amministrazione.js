...
const AmministrazioneModule = {
    ...
    render: function() {
        ...
        container.innerHTML = `
            ...
            <div class="dashboard-grid" style="grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); margin-bottom: 24px;">
                <div class="card">
                    <div class="card-header">
                        <span class="card-title">Totale Crediti</span>
                ...
                <div class="card">
                    <div class="card-header">
                        <span class="card-title">Totale Debiti</span>
            ...
            <div class="card">
                <div class="card-header">
                    <span class="card-title">Gestione Clienti</span>
    ...
    // --- DATI & CALCOLI ---
    getClients: function() {
        try { return JSON.parse(localStorage.getItem('polaris_clients') || '[]'); } catch (e) { return []; }
    },

    getLastTransaction: function(client) {
        if (!client.transactions || client.transactions.length === 0) return null;
        return [...client.transactions].sort((a, b) => new Date(b.date) - new Date(a.date))[0];
    },
    ...
    getFilteredClients: function() {
        ...
        clients.sort((a, b) => {
            ...
            switch(column) {
                ...
                case 'lastDate': 
                    const txA = this.getLastTransaction(a); const txB = this.getLastTransaction(b);
                    valA = txA ? new Date(txA.date).getTime() : 0; valB = txB ? new Date(txB.date).getTime() : 0;
    ...
    calculateStats: function(clients) {
        let credit = 0, debit = 0;
        clients.forEach(c => { if (c.balance > 0) credit += c.balance; else debit += c.balance; });
        return { totalCredit: credit, totalDebit: Math.abs(debit), clientCount: clients.length };
    },

    handleSort: function(col) {
        if (this.sort.column === col) { this.sort.direction = this.sort.direction === 'asc' ? 'desc' : 'asc'; } 
        else { this.sort.column = col; this.sort.direction = 'asc'; }
        this.render();
    },
    ...
    renderTable: function(clients) {
        ...
        const pageItems = clients.slice(start, end);
        const th = (label, col, align = 'left') => {
            const isActive = this.sort.column === col;
            const icon = isActive ? (this.sort.direction === 'asc' ? 'chevron-up' : 'chevron-down') : 'chevrons-up-down';
            const color = isActive ? 'var(--text-main)' : 'var(--text-secondary)';
        ...
        pageItems.forEach(c => {
            ...
            const lastDate = lastTx ? new Date(lastTx.date).toLocaleDateString() : '-';
            let lastAmount = '-';
            if (lastTx) {
                const amtClass = lastTx.amount > 0 ? 'text-success' : 'text-danger';
                lastAmount = `<span class="${amtClass}">${lastTx.amount.toLocaleString('it-IT', {style:'currency', currency:'EUR'})}</span>`;
            }
    ...
    openClientModal: function(idToEdit = null) {
        ...
        if (!idToEdit) {
            ...
            const bodyHTML = `
                <form id="form-client">
                    <div style="margin-bottom: 20px;">
                        ...
                        <input type="text" id="inp-name" class="nav-link" placeholder="Inserisci nome..." style="width:100%; border:1px solid var(--border-color); border-radius:var(--radius-input);">
                    </div>
                </form>
            `;
            ...
            setTimeout(() => {
                document.getElementById('btn-save-client').addEventListener('click', () => this.saveNewClient());
                document.getElementById('btn-cancel-client').addEventListener('click', () => this.closeModal());
                document.getElementById('inp-name').focus();
            }, 0);

        } else {
            // GESTIONE CLIENTE (800px)
            ...
            const bodyHTML = `
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px; padding:15px; background-color:var(--primary-light-bg); border-radius:var(--radius-input);">
                    ...
                    <div style="display:flex; gap:10px;">
                        <button class="action-btn btn-secondary" onclick="AmministrazioneModule.printStatement('${client.id}')" title="Stampa Estratto"><i data-lucide="printer" style="width:16px;"></i></button>
                        <button class="action-btn btn-secondary" onclick="AmministrazioneModule.confirmSaldo('${client.id}')" title="Chiudi Conto">SALDO</button>
                        <button class="action-btn btn-delete" onclick="AmministrazioneModule.deleteClient('${client.id}')" title="Elimina Cliente"><i data-lucide="trash-2" style="width:16px;"></i></button>
                    </div>
                </div>
                ...
                <div style="border:1px solid var(--border-color); border-radius:var(--radius-input); padding:15px; margin-bottom:20px; background-color: var(--bg-app);">
                    ...
                    <div style="display:flex; gap:10px; margin-top:10px; font-size:0.8rem;">
                        <button class="action-btn btn-secondary" style="padding:5px 15px; height:auto; font-size:0.8rem;" onclick="document.getElementById('tx-desc').value='Carburante'; document.getElementById('tx-amount').focus();">Carburante</button>
                        <button class="action-btn btn-secondary" style="padding:5px 15px; height:auto; font-size:0.8rem;" onclick="document.getElementById('tx-desc').value='Acconto'; document.getElementById('tx-amount').focus();">Acconto</button>
                    </div>
                </div>

                <div style="max-height:240px; overflow-y:auto; border-top:1px solid var(--border-color);">
    ...
    confirmSaldo: function(clientId) {
        ...
        const bodyHTML = `
            <div style="text-align:center; padding:10px;">
                <p style="color:var(--text-main); margin-bottom:10px;">Vuoi saldare il conto di <strong>${client.name}</strong>?</p>
                <p style="font-size:0.9rem; color:var(--col-destructive);">Questa operazione cancellerà tutto lo storico transazioni.</p>
            </div>
        `;

        const footerHTML = `
            <div class="btn-group">
                <button id="btn-cancel-saldo" class="action-btn btn-cancel" style="background:var(--bg-app); color:var(--text-main); border:1px solid var(--border-color); box-shadow:none;">ANNULLA</button>
                <button id="btn-print-saldo" class="action-btn btn-secondary">STAMPA</button>
                <button id="btn-confirm-saldo" class="action-btn btn-delete">ELIMINA</button>
            </div>
        `;

        this.openModal('Saldo Cliente', bodyHTML, footerHTML, '450px');

        setTimeout(() => {
            document.getElementById('btn-cancel-saldo').onclick = () => this.openClientModal(clientId);
            document.getElementById('btn-print-saldo').onclick = () => this.printStatement(clientId);
            document.getElementById('btn-confirm-saldo').onclick = () => this.executeSaldo(clientId);
        }, 0);
    },

    executeSaldo: function(clientId) {
        const clients = this.getClients();
        const idx = clients.findIndex(c => c.id === clientId);
        if (idx !== -1) {
            clients[idx].balance = 0;
            clients[idx].transactions = [];
            localStorage.setItem('polaris_clients', JSON.stringify(clients));
            showNotification("Conto saldato e storico azzerato.", 'success');
    ...
    saveNewClient: function() {
        ...
        if (!name) { showNotification("Inserisci un nome", 'error'); return; }
        const clients = this.getClients();
        clients.push({ id: Date.now().toString(), name: name, balance: 0, transactions: [] });
        localStorage.setItem('polaris_clients', JSON.stringify(clients));
        this.closeModal();
    ...
    addTransaction: function(clientId) {
        ...
        if (!desc || isNaN(amount) || amount === 0) { showNotification("Dati non validi", 'error'); return; }
        const clients = this.getClients();
        const clientIdx = clients.findIndex(c => c.id === clientId);
        if (clientIdx === -1) return;

        let finalAmount = amount;
        if (desc.toLowerCase().includes('carburante') && amount > 0) finalAmount = -amount;
    ...
    editTransaction: function(clientId, txId) {
        const clients = this.getClients();
        const cIdx = clients.findIndex(c => c.id === clientId);
        if (cIdx === -1) return;
        const tx = clients[cIdx].transactions.find(t => t.id === txId);
        if (!tx) return;
        clients[cIdx].balance -= tx.amount;
        clients[cIdx].transactions = clients[cIdx].transactions.filter(t => t.id !== txId);
        localStorage.setItem('polaris_clients', JSON.stringify(clients));
        this.openClientModal(clientId);
        setTimeout(() => {
    ...
    deleteTransaction: function(clientId, txId) {
        ...
        const cIdx = clients.findIndex(c => c.id === clientId);
        if (cIdx === -1) return;
        const txIdx = clients[cIdx].transactions.findIndex(t => t.id === txId);
        if (txIdx !== -1) {
            clients[cIdx].balance -= clients[cIdx].transactions[txIdx].amount;
            clients[cIdx].transactions.splice(txIdx, 1);
            localStorage.setItem('polaris_clients', JSON.stringify(clients));
            this.openClientModal(clientId);
            this.render();
        }
    ...
    deleteClient: function(id) {
        if(!confirm("Eliminare INTERO cliente e storico?")) return;
    ...
    printStatement: function(id) {
        ...
        w.document.write(`<html><head><title>Estratto Conto</title><style>body{font-family:sans-serif;padding:20px}table{width:100%;border-collapse:collapse;margin-top:20px}th,td{border:1px solid #ddd;padding:8px;text-align:left}th{background-color:#f2f2f2}.text-right{text-align:right}</style></head><body><h2>Estratto Conto: ${client.name}</h2><p>Data: ${new Date().toLocaleDateString()}</p><table><thead><tr><th>Data</th><th>Descrizione</th><th class="text-right">Importo</th></tr></thead><tbody>${client.transactions.map(t=>`<tr><td>${new Date(t.date).toLocaleDateString()}</td><td>${t.description}</td><td class="text-right">${t.amount.toLocaleString('it-IT',{style:'currency',currency:'EUR'})}</td></tr>`).join('')}</tbody><tfoot><tr><th colspan="2" class="text-right">SALDO FINALE</th><th class="text-right">${client.balance.toLocaleString('it-IT',{style:'currency',currency:'EUR'})}</th></tr></tfoot></table><script>window.print();</script></body></html>`);
        w.document.close();
    },
    ...
    printList: function() {
        ...
        for(let i=0; i<clients.length; i+=2) {
            ...
            const cell1 = c1 ? `<td>${c1.name}</td><td class="text-right">${c1.balance.toLocaleString('it-IT', {style:'currency', currency:'EUR'})}</td>` : `<td></td><td></td>`;
            const cell2 = c2 ? `<td>${c2.name}</td><td class="text-right">${c2.balance.toLocaleString('it-IT', {style:'currency', currency:'EUR'})}</td>` : `<td></td><td></td>`;
        ...
        w.document.write(`
            ...
            <head>
                ...
                <style>
                    body { font-family: 'Montserrat', sans-serif; font-size: 10pt; padding: 20px; }
                    h2 { text-align: center; margin-bottom: 5px; text-transform: uppercase; }
                    p { text-align: center; margin-top: 0; margin-bottom: 20px; font-size: 9pt; color: #666; }
                    table { width: 100%; border-collapse: collapse; }
                    th, td { border: 1px solid #ccc; padding: 6px 8px; font-size: 9pt; vertical-align: middle; }
                    th { background-color: #f0f0f0; font-weight: 600; text-align: left; }
                    .text-right { text-align: right; }
                    td:nth-child(1), td:nth-child(3) { width: 35%; }
                    td:nth-child(2), td:nth-child(4) { width: 15%; font-weight: bold; }
                    @media print { @page { margin: 1cm; } }
                </style>
            </head>
            <body>
                <h2>Riepilogo Clienti a credito</h2>
                <p>Data: ${new Date().toLocaleDateString('it-IT')}</p>
                <table>
                    <thead><tr><th>Cliente</th><th class="text-right">Saldo</th><th>Cliente</th><th class="text-right">Saldo</th></tr></thead>
                    <tbody>${rows}</tbody>
                </table>
    ...
    attachMainListeners: function() {
        document.getElementById('search-client').addEventListener('input', (e) => {
            this.currentFilter = e.target.value; this.currentPage = 1; this.render();
            const inp = document.getElementById('search-client'); inp.focus(); const val=inp.value; inp.value=''; inp.value=val;
        });
        const btnClear = document.getElementById('btn-clear-search');
        if(btnClear) btnClear.addEventListener('click', () => { this.currentFilter = ''; this.currentPage = 1; this.render(); document.getElementById('search-client').focus(); });
    ...
    exportData: function() {
        try { const data = this.getClients(); const a = document.createElement('a'); a.href = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(data)); a.download = "polaris_amministrazione.json"; document.body.appendChild(a); a.click(); a.remove(); } catch (e) { showNotification("Errore Export", 'error'); }
    },
    ...
    openModal: function(title, bodyHTML, footerHTML, maxWidth='800px') {
        const m = document.getElementById('modal-overlay'); const mb = document.querySelector('.modal-box'); const bdy = document.getElementById('modal-body'); document.getElementById('modal-title').innerText = title; bdy.innerHTML = bodyHTML; mb.style.maxWidth = maxWidth; const of = mb.querySelector('.modal-footer'); if(of) of.remove(); if(footerHTML) { const f = document.createElement('div'); f.className = 'modal-footer'; f.innerHTML = footerHTML; mb.appendChild(f); } m.classList.remove('hidden'); lucide.createIcons();
    },
