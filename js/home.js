/* ==========================================================================
   MODULO: Home Dashboard (js/home.js) - Fixes & Clean Design
   ========================================================================== */
(function() {
    'use strict';
    const HomeModule = {
        ...
        renderStats() {
            ...
            if (c1 && c1.children.length === 0) {
                ...
                // Card statistiche con header e divisore
                c1.innerHTML = `
                    ${this.renderStatCard('stat-revenue', 'Fatturato Oggi', 'bg-green-600', 'euro', 'val-revenue', App.formatCurrency(s.revenue))}
                    ${this.renderStatCard('stat-margin', 'Margine Stimato', 'bg-yellow-500', 'trending-up', 'val-margin', App.formatCurrency(s.margin))}
                    ${this.renderStatCard('stat-liters', 'Totale Litri', 'bg-blue-600', 'droplet', 'val-liters', App.formatNumber(s.totalLiters))}
                    ${this.renderStatCard('stat-served', '% Servito', 'bg-purple-600', 'user-check', 'val-served', s.servitoPerc + '%')}
                `;
                lucide.createIcons();
        ...
        // Helper per le Stat Cards con Header Standard
        renderStatCard(id, title, iconBg, iconName, valId, value) {
            return `
        ...
        renderActivitiesAndOrders() {
            ...
            if(actList) {
                if(!acts.length) actList.innerHTML = '<div class="flex flex-col items-center justify-center h-full pb-4 text-gray-400 dark:text-gray-500"><i data-lucide="clipboard-list" class="w-10 h-10 mb-2 opacity-50"></i><p class="text-sm">Nessuna attività oggi</p></div>';
                    ...
                    else { borderClass = 'border-green-200 dark:border-green-800'; bgClass = 'bg-green-50 dark:bg-green-900/20'; iconColor = 'text-green-600 dark:text-green-400'; labelColor = 'text-green-700 dark:text-green-300'; }
