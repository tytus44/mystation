...
let homeState = {
    ...
    calculator: {
        display: '0', equation: '', firstOperand: null,
...
function setupHomeEventListeners() {
    ...
    document.getElementById('calendar-today-btn')?.addEventListener('click', () => {
        ...
        homeState.calendar.selectedDate = `${year}-${month}-${day}`; renderCalendar.call(app);
    });
...
function setActiveHomeCardTab(tab) {
    ...
    document.querySelectorAll('[data-tab]').forEach(btn => { const isActive = btn.dataset.tab === tab; btn.classList.toggle('btn-primary', isActive); btn.classList.toggle('active', isActive); btn.classList.toggle('btn-secondary', !isActive); });
...
function calcolaIva() {
    ...
    if (lordo === null || isNaN(lordo) || lordo <= 0) homeState.ivaCalculator.risultati = { imponibile: 0, iva: 0 };
    else { const aliquota = 22 / 100; const imponibile = lordo / (1 + aliquota); homeState.ivaCalculator.risultati.imponibile = imponibile; homeState.ivaCalculator.risultati.iva = lordo - imponibile; }
...
function updateIvaDisplay() {
    const lordoEl = document.getElementById('iva-lordo'); const imponibileEl = document.getElementById('iva-imponibile'); const ivaEl = document.getElementById('iva-iva');
    if (lordoEl) lordoEl.textContent = this.formatCurrency(homeState.ivaCalculator.importoLordo || 0);
    if (imponibileEl) imponibileEl.textContent = this.formatCurrency(homeState.ivaCalculator.risultati.imponibile);
...
function calcolaTotaleBanconote() {
    const app = this; const counter = homeState.banconoteCounter; const tagli = [500, 200, 100, 50, 20, 10];
    let totale = 0; let numeroBanconote = 0;
    tagli.forEach(taglio => {
        const quantita = parseInt(counter[taglio], 10) || 0; const subtotale = quantita * taglio;
        totale += subtotale; numeroBanconote += quantita;
        const subtotaleEl = document.getElementById(`banconote-subtotal-${taglio}`); if (subtotaleEl) subtotaleEl.textContent = app.formatCurrency(subtotale);
    });
    homeState.banconoteCounter.total = totale; homeState.banconoteCounter.count = numeroBanconote;
    const totaleEl = document.getElementById('banconote-total'); if (totaleEl) totaleEl.textContent = this.formatCurrency(totale);
    const countEl = document.getElementById('banconote-count'); if (countEl) countEl.textContent = this.formatInteger(numeroBanconote);
}
// Fine funzione calcolaTotaleBanconote

// Inizio funzione renderBanconoteInputs
function renderBanconoteInputs() {
    const app = this; const container = document.getElementById('banconote-inputs-container'); if (!container) return;
    const tagli = [500, 200, 100, 50, 20, 10];
    container.innerHTML = tagli.map(taglio => {
        const quantita = homeState.banconoteCounter[taglio] || 0; const subtotale = quantita * taglio;
        return `
            <div class="flex items-center justify-between p-3 rounded-lg" style="border: 1px solid var(--border-primary); background-color: var(--bg-secondary);">
                <div style="width: 100px; text-align: center;"><span class="text-md font-medium text-primary">€ ${taglio}</span></div>
                <div style="width: 200px;"><div class="number-input-group">
                ...
                <div style="width: 150px; text-align: center;"><span id="banconote-subtotal-${taglio}" class="text-md font-bold text-success">${app.formatCurrency(subtotale)}</span></div>
            </div>`;
    }).join('');
...
function incrementBanconota(taglio) {
    const taglioNum = parseInt(taglio, 10); let quantita = homeState.banconoteCounter[taglioNum] || 0; quantita++; homeState.banconoteCounter[taglioNum] = quantita;
...
function decrementBanconota(taglio) {
    const taglioNum = parseInt(taglio, 10); let quantita = homeState.banconoteCounter[taglioNum] || 0; if (quantita > 0) quantita--;
...
function updateBanconotaInput(taglio, quantita) {
    const app = this; const quantitaInput = document.getElementById(`banconota-quantita-${taglio}`); if (quantitaInput) quantitaInput.value = app.formatInteger(quantita);
...
function renderCalendar() {
    const date = homeState.calendar.currentDate; const year = date.getFullYear(); const month = date.getMonth();
    const monthNames = ['Gennaio', 'Febbraio', 'Marzo', 'Aprile', 'Maggio', 'Giugno', 'Luglio', 'Agosto', 'Settembre', 'Ottobre', 'Novembre', 'Dicembre'];
    homeState.calendar.monthYear = `${monthNames[month]} ${year}`;
    const monthYearEl = document.getElementById('calendar-month-year'); if (monthYearEl) monthYearEl.textContent = homeState.calendar.monthYear;
    const firstDayOfMonth = new Date(year, month, 1).getDay(); const firstDayIndex = (firstDayOfMonth + 6) % 7; const lastDateOfMonth = new Date(year, month + 1, 0).getDate();
    let daysArray = []; for (let i = 0; i < firstDayIndex; i++) daysArray.push({ value: '', isToday: false, isHoliday: false, isSunday: false, todos: [], date: null });
    const oggi = new Date(); const oggiYear = oggi.getFullYear(); const oggiMonth = String(oggi.getMonth() + 1).padStart(2, '0'); const oggiDay = String(oggi.getDate()).padStart(2, '0'); const oggiString = `${oggiYear}-${oggiMonth}-${oggiDay}`;
    for (let i = 1; i <= lastDateOfMonth; i++) {
        const monthString = String(month + 1).padStart(2, '0'); const dayString = String(i).padStart(2, '0'); const dateString = `${year}-${monthString}-${dayString}`;
        const dataCorrente = new Date(year, month, i); const isToday = dateString === oggiString; const isSunday = isDomenica(dataCorrente); const isHoliday = isFestivaItaliana.call(this, dataCorrente);
    ...
    homeState.calendar.days = daysArray; renderCalendarDays();
}
...
function renderCalendarDays() {
    ...
    homeState.calendar.days.forEach(day => {
        ...
        const dayNumber = document.createElement('span'); dayNumber.textContent = day.value; dayEl.appendChild(dayNumber);
        if (day.date) dayEl.dataset.date = day.date; if (day.isToday) dayEl.classList.add('today'); if (day.isHoliday) dayEl.classList.add('holiday'); if (day.isSunday) dayEl.classList.add('sunday');
        if (day.date === homeState.calendar.selectedDate) dayEl.classList.add('selected'); if (!day.value) dayEl.classList.add('empty');
        if (day.todos && day.todos.length > 0) {
            dayEl.classList.add('has-todo'); const dotsContainer = document.createElement('div'); dotsContainer.className = 'todo-dots-container';
            const importanceOrder = { 'urgent': 1, 'priority': 2, 'standard': 3 }; const sortedTodos = [...day.todos].sort((a, b) => (importanceOrder[a.color] || 3) - (importanceOrder[b.color] || 3));
            const colorToDotClass = { 'urgent': 'dot-1', 'priority': 'dot-2', 'standard': 'dot-3' };
            sortedTodos.slice(0, 3).forEach(todo => { const dot = document.createElement('span'); const dotClass = colorToDotClass[todo.color] || 'dot-3'; dot.className = `todo-dot ${dotClass}`; if (todo.completed) dot.classList.add('completed'); dotsContainer.appendChild(dot); });
...
function renderTodos() {
    ...
    if (todoList) { const sortedTodos = [...homeState.todos].sort((a, b) => new Date(a.dueDate) - new Date(b.dueDate));
        todoList.innerHTML = ` ${sortedTodos.map(todo => `<div class="todo-item ${todo.completed ? 'completed' : ''} color-${todo.color || 'standard'}" data-todo-id="${todo.id}"><div class="flex-grow"><span>${todo.text}</span> ${todo.dueDate ? `<div class="text-sm text-secondary">Scadenza: ${app.formatDate(todo.dueDate)}</div>` : ''}</div><button class="delete-btn ml-auto" data-todo-id="${todo.id}"><i data-lucide="x" class="w-4 h-4"></i></button></div>`).join('')} ${homeState.todos.length === 0 ? '<p class="text-secondary">Nessuna attività. Clicca un giorno sul calendario per aggiungerne una!</p>' : ''} `;
    } app.refreshIcons();
}
