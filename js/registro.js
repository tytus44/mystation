...
function renderRegistroListView(container) {
    ...
    container.innerHTML = `
        <div class="space-y-6">
            <div id="registry-stats-container" class="stats-grid no-print"></div>
            <div class="filters-bar no-print">
                ...
                <div class="filter-group">
                    <div class="btn-group">
                        <button class="btn ${registroState.registryTimeFilter === 'all' ? 'btn-primary active' : 'btn-secondary'}" data-time-filter="all">Tutto</button>
                        <button class="btn ${registroState.registryTimeFilter === 'month' ? 'btn-primary active' : 'btn-secondary'}" data-time-filter="month">Mese</button>
                        <button class="btn ${registroState.registryTimeFilter === 'quarter' ? 'btn-primary active' : 'btn-secondary'}" data-time-filter="quarter">Trimestre</button>
                        <button class="btn ${registroState.registryTimeFilter === 'semester' ? 'btn-primary active' : 'btn-secondary'}" data-time-filter="semester">Semestre</button>
                    </div>
                </div>
                <button id="new-carico-btn" class="btn btn-primary"><i data-lucide="truck"></i> Nuovo Carico</button>
            </div>
            <div class="card no-print">
                <div class="card-header"><h2 class="card-title">Riepilogo totali ${new Date().getFullYear()}</h2></div>
                <div class="table-container">
                    <table class="table">
                        <thead><tr><th>Prodotto</th><th>Carico</th><th class="text-success">Differenza +</th><th class="text-danger">Differenza -</th><th>Anno Prec.</th><th>Contabile</th></tr></thead>
                        <tbody>
                            <tr>
                                <td class="font-medium text-primary">Benzina</td><td>${app.formatInteger(summary.benzina.carico)}</td><td class="text-success">${app.formatInteger(summary.benzina.diff_pos)}</td><td class="text-danger">${app.formatInteger(summary.benzina.diff_neg)}</td>
                                <td><input type="number" id="prev-year-benzina" class="form-control w-24" value="${app.state.data.previousYearStock.benzina}" autocomplete="off"></td>
                            ...
                            <tr>
                                <td class="font-medium text-primary">Gasolio</td><td>${app.formatInteger(summary.gasolio.carico)}</td><td class="text-success">${app.formatInteger(summary.gasolio.diff_pos)}</td><td class="text-danger">${app.formatInteger(summary.gasolio.diff_neg)}</td>
                                <td><input type="number" id="prev-year-gasolio" class="form-control w-24" value="${app.state.data.previousYearStock.gasolio}" autocomplete="off"></td>
                            ...
                            <tr>
                                <td class="font-medium text-primary">Diesel+</td><td>${app.formatInteger(summary.dieselPlus.carico)}</td><td class="text-success">${app.formatInteger(summary.dieselPlus.diff_pos)}</td><td class="text-danger">${app.formatInteger(summary.dieselPlus.diff_neg)}</td>
                                <td><input type="number" id="prev-year-dieselPlus" class="form-control w-24" value="${app.state.data.previousYearStock.dieselPlus}" autocomplete="off"></td>
                            ...
                            <tr>
                                <td class="font-medium text-primary">Hvolution</td><td>${app.formatInteger(summary.hvolution.carico)}</td><td class="text-success">${app.formatInteger(summary.hvolution.diff_pos)}</td><td class="text-danger">${app.formatInteger(summary.hvolution.diff_neg)}</td>
                                <td><input type="number" id="prev-year-hvolution" class="form-control w-24" value="${app.state.data.previousYearStock.hvolution}" autocomplete="off"></td>
                        ...
                        <tfoot class="font-bold text-primary bg-secondary">
                            <tr><td>Totale</td><td>${app.formatInteger(summary.benzina.carico + summary.gasolio.carico + summary.dieselPlus.carico + summary.hvolution.carico)}</td><td class="text-success">${app.formatInteger(summary.benzina.diff_pos + summary.gasolio.diff_pos + summary.dieselPlus.diff_pos + summary.hvolution.diff_pos)}</td><td class="text-danger">${app.formatInteger(summary.benzina.diff_neg + summary.gasolio.diff_neg + summary.dieselPlus.diff_neg + summary.hvolution.diff_neg)}</td><td>-</td>
                                <td>${app.formatInteger((summary.benzina.carico + (app.state.data.previousYearStock.benzina || 0) + summary.benzina.diff_pos + summary.benzina.diff_neg) + (summary.gasolio.carico + (app.state.data.previousYearStock.gasolio || 0) + summary.gasolio.diff_pos + summary.gasolio.diff_neg) + (summary.dieselPlus.carico + (app.state.data.previousYearStock.dieselPlus || 0) + summary.dieselPlus.diff_pos + summary.dieselPlus.diff_neg) + (summary.hvolution.carico + (app.state.data.previousYearStock.hvolution || 0) + summary.hvolution.diff_pos + summary.hvolution.diff_neg))}</td></tr>
                        </tfoot>
                    </table>
                </div>
            </div>
                    ...
                    <div class="table-container">
                        <table class="table" id="registry-table">
                            <thead><tr>
                                ...
                                <th><button class="flex items-center" data-sort="autistaName">Autista <i data-lucide="arrow-up-down" class="w-3 h-3 ml-1.5"></i></button></th>
                                <th>Benzina</th><th>Gasolio</th><th>Diesel+</th><th>Hvolution</th><th class="text-right">Azioni</th>
                            </tr></thead>
        ...
        <div class="modal-body">
            <div class="space-y-6">
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="form-group"><label class="form-label">Nome Autista</label><input type="text" id="carico-autista" class="form-control" placeholder="Cognome Nome" value="${registroState.registryForm.autistaName}" autocomplete="off"></div>
                </div>
                <div class="table-container"><table class="table">
                    <thead><tr>
                        <th>Prodotto</th>
                        <th class="text-center" style="width: 220px;">Carico</th>
