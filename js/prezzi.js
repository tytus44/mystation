...
// --- CORRETTO FONT SIZE (RIMOSSO OGNI text-xs e commenti errati) ---
// =============================================
                ...
                <div class="grid grid-cols-2 gap-6">
                    <div class="stat-card" style="background-color: #10b981; border-color: #059669;">
                        <div class="stat-content">
                            <div style="color: #ffffff;">Benzina</div>
                            <div class="stat-value" style="color: #ffffff;">${app.formatCurrency(latestPrices.benzina, true)}</div>
                            <div style="margin-top: 0.25rem; color: rgba(255, 255, 255, 0.9);">Servito: ${app.formatCurrency(servedPrices.benzina, true)}</div>
                        </div>
                    ...
                    <div class="stat-card" style="background-color: #f59e0b; border-color: #d97706;">
                         <div class="stat-content">
                            <div style="color: #ffffff;">Gasolio</div>
                            <div class="stat-value" style="color: #ffffff;">${app.formatCurrency(latestPrices.gasolio, true)}</div>
                            <div style="margin-top: 0.25rem; color: rgba(255, 255, 255, 0.9);">Servito: ${app.formatCurrency(servedPrices.gasolio, true)}</div>
                        </div>
                    ...
                    <div class="stat-card" style="background-color: #FF204E; border-color: #DC1C44;">
                         <div class="stat-content">
                            <div style="color: #ffffff;">Diesel+</div>
                            <div class="stat-value" style="color: #ffffff;">${app.formatCurrency(latestPrices.dieselPlus, true)}</div>
                            ${latestPrices.dieselPlus > 0 ?
                                `<div style="margin-top: 0.25rem; color: rgba(255, 255, 255, 0.9);">Servito: ${app.formatCurrency(servedPrices.dieselPlus, true)}</div>` : ''}
                        </div>
                    ...
                    <div class="stat-card" style="background-color: #06b6d4; border-color: #0891b2;">
                         <div class="stat-content">
                            <div style="color: #ffffff;">Hvolution</div>
                            <div class="stat-value" style="color: #ffffff;">${app.formatCurrency(latestPrices.hvolution, true)}</div>
                            ${latestPrices.hvolution > 0 ?
                                `<div style="margin-top: 0.25rem; color: rgba(255, 255, 255, 0.9);">Servito: ${app.formatCurrency(servedPrices.hvolution, true)}</div>` : ''}
                        </div>
...
// === FUNZIONI PER CREARE HTML DEI FORM ===
    ...
    return `
        ...
        <div class="card-body" style="padding: 1rem;">
            <div style="display: flex; flex-direction: column; gap: 1rem;">
                <div class="form-group" style="margin-bottom: 0;"><input type="text" id="listino-date" class="form-control" placeholder="Data (gg.mm.aaaa)" value="${prezziState.listinoForm.date}" autocomplete="off"></div>
                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem;">
                    <div class="product-box" style="border-color: rgba(16, 185, 129, 0.65); padding: 0.75rem;"><label class="form-label font-medium" style="color: var(--color-success); margin-bottom: 0.5rem;">Benzina</label><input type="number" step="0.001" id="listino-benzina" class="form-control" placeholder="1.000" value="${prezziState.listinoForm.benzina}" autocomplete="off"></div>
                    <div class="product-box" style="border-color: rgba(245, 158, 11, 0.65); padding: 0.75rem;"><label class="form-label font-medium" style="color: var(--color-warning); margin-bottom: 0.5rem;">Gasolio</label><input type="number" step="0.001" id="listino-gasolio" class="form-control" placeholder="1.000" value="${prezziState.listinoForm.gasolio}" autocomplete="off"></div>
                    <div class="product-box" style="border-color: rgba(255, 32, 78, 0.65); padding: 0.75rem;"><label class="form-label font-medium" style="color: var(--color-danger); margin-bottom: 0.5rem;">Diesel+</label><input type="number" step="0.001" id="listino-dieselPlus" class="form-control" placeholder="1.000" value="${prezziState.listinoForm.dieselPlus}" autocomplete="off"></div>
                    <div class="product-box" style="border-color: rgba(8, 145, 178, 0.65); padding: 0.75rem;"><label class="form-label font-medium" style="color: rgb(8, 145, 178); margin-bottom: 0.5rem;">Hvolution</label><input type="number" step="0.001" id="listino-hvolution" class="form-control" placeholder="1.000" value="${prezziState.listinoForm.hvolution}" autocomplete="off"></div>
                    <div class="product-box" style="border-color: rgba(107, 114, 128, 0.65); padding: 0.75rem;"><label class="form-label font-medium" style="color: var(--color-secondary); margin-bottom: 0.5rem;">AdBlue</label><input type="number" step="0.001" id="listino-adblue" class="form-control" placeholder="1.000" value="${prezziState.listinoForm.adblue}" autocomplete="off"></div>
                </div>
                <div class="flex items-center justify-end space-x-4" style="margin-top: 0.5rem;"><button id="cancel-listino-btn-bottom" class="btn btn-secondary">Annulla</button><button id="save-listino-btn" class="btn btn-success">Salva Listino</button></div>
            </div>
        </div>`;
}
...
function getConcorrenzaFormHTML() {
    ...
    if (!prezziState.concorrenzaForm.myoil.benzina && !prezziState.concorrenzaForm.myoil.gasolio) {
    ...
    return `
        <div class="card-header"><h2 class="card-title">Aggiorna Prezzi Concorrenza</h2></div>
        <div class="card-body">
            <div class="space-y-6">
                <div class="form-group max-w-sm"><input type="text" id="concorrenza-date" class="form-control" placeholder="Data (gg.mm.aaaa)" value="${prezziState.concorrenzaForm.date}" autocomplete="off"></div>
                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem;">
                    <div class="product-box" style="border-color: rgba(139, 92, 246, 0.65);"><h4 class="product-title text-center" style="color: #8b5cf6">MyOil</h4><div class="form-group"><label class="form-label">Benzina</label><input type="number" step="0.001" id="myoil-benzina" class="form-control" value="${prezziState.concorrenzaForm.myoil.benzina}" autocomplete="off"></div><div class="form-group"><label class="form-label">Gasolio</label><input type="number" step="0.001" id="myoil-gasolio" class="form-control" value="${prezziState.concorrenzaForm.myoil.gasolio}" autocomplete="off"></div></div>
                    <div class="product-box" style="border-color: rgba(255, 32, 78, 0.65);"><h4 class="product-title text-center" style="color: var(--color-danger)">Esso</h4><div class="form-group"><label class="form-label">Benzina</label><input type="number" step="0.001" id="esso-benzina" class="form-control" value="${prezziState.concorrenzaForm.esso.benzina}" autocomplete="off"></div><div class="form-group"><label class="form-label">Gasolio</label><input type="number" step="0.001" id="esso-gasolio" class="form-control" value="${prezziState.concorrenzaForm.esso.gasolio}" autocomplete="off"></div></div>
                    <div class="product-box" style="border-color: rgba(8, 145, 178, 0.65);"><h4 class="product-title text-center" style="color: rgb(8, 145, 178)">Q8</h4><div class="form-group"><label class="form-label">Benzina</label><input type="number" step="0.001" id="q8-benzina" class="form-control" value="${prezziState.concorrenzaForm.q8.benzina}" autocomplete="off"></div><div class="form-group"><label class="form-label">Gasolio</label><input type="number" step="0.001" id="q8-gasolio" class="form-control" value="${prezziState.concorrenzaForm.q8.gasolio}" autocomplete="off"></div></div>
                </div>
                <div class="flex items-center justify-end space-x-4"><button id="cancel-concorrenza-btn-bottom" class="btn btn-secondary">Annulla</button><button id="save-concorrenza-btn" class="btn btn-success">Salva Prezzi</button></div>
            </div>
        </div>`;
}
...
function setupPrezziEventListeners() {
    ...
    if (newListinoBtn) newListinoBtn.addEventListener('click', () => showCreateListino.call(app));
    const updateConcorrenzaBtn = document.getElementById('update-concorrenza-btn');
    if (updateConcorrenzaBtn) updateConcorrenzaBtn.addEventListener('click', () => showUpdateConcorrenza.call(app));
...
function setupListinoFormEventListeners() {
    ...
    if (dateInput) {
        dateInput.addEventListener('input', (e) => prezziState.listinoForm.date = e.target.value);
        dateInput.focus();
    }
    ['benzina', 'gasolio', 'dieselPlus', 'hvolution', 'adblue'].forEach(product => {
        const input = document.getElementById(`listino-${product}`);
        if (input) input.addEventListener('input', (e) => prezziState.listinoForm[product] = e.target.value);
    });
    const saveBtn = document.getElementById('save-listino-btn');
    if (saveBtn) saveBtn.addEventListener('click', () => saveListino.call(app));
    const cancelBtnBottom = document.getElementById('cancel-listino-btn-bottom');
    if (cancelBtnBottom) cancelBtnBottom.addEventListener('click', () => app.hideFormModal());
}
...
function setupConcorrenzaFormEventListeners() {
    ...
    if (dateInput) {
        dateInput.addEventListener('input', (e) => prezziState.concorrenzaForm.date = e.target.value);
        dateInput.focus();
    }
    ...
    if (saveBtn) saveBtn.addEventListener('click', () => saveConcorrenza.call(app));
...
// === FUNZIONI DI ORDINAMENTO E DATI ===
function sortPrices(column) {
    if (prezziState.priceSort.column === column) {
        ...
        prezziState.priceSort.direction = 'desc';
    }
...
function sortedPriceHistory() {
    ...
    return [...this.state.data.priceHistory].sort((a, b) => {
        ...
        if (dateComparison !== 0) return dateComparison * dir;
...
function currentPrices() {
    ...
    if (!listini || listini.length === 0) return { benzina: 0, gasolio: 0, dieselPlus: 0, hvolution: 0, adblue: 0 };
    return [...listini].sort((a, b) => new Date(b.date) - new Date(a.date))[0];
}

function competitorPrices() {
    const concorrenza = this.state.data.competitorPrices;
    if (!concorrenza || concorrenza.length === 0) return { myoil: { benzina: 0, gasolio: 0 }, esso: { benzina: 0, gasolio: 0 }, q8: { benzina: 0, gasolio: 0 } };
    return [...concorrenza].sort((a, b) => new Date(b.date) - new Date(a.date))[0];
}

// === FUNZIONI PER RESETTARE E AGGIORNARE I FORM ===
...
function resetConcorrenzaForm() {
    prezziState.concorrenzaForm = {
        date: this.formatToItalianDate(new Date().toISOString()),
        myoil: { benzina: '', gasolio: '' }, esso: { benzina: '', gasolio: '' }, q8: { benzina: '', gasolio: '' }
    };
}

function updateConcorrenzaFormValue(path, value) {
    const keys = path.split('.');
    let current = prezziState.concorrenzaForm;
    for (let i = 0; i < keys.length - 1; i++) current = current[keys[i]];
    current[keys[keys.length - 1]] = value === '' ? null : value;
}

// === FUNZIONI PER SALVARE E ELIMINARE ===
function saveListino() {
    const app = getApp();
    const form = prezziState.listinoForm;
    if (!form.date || !form.benzina || !form.gasolio) return app.showNotification('Data, prezzo benzina e gasolio sono obbligatori');
    if (!app.validateItalianDate(form.date)) return app.showNotification('Formato data non valido. Usa gg.mm.aaaa');
    const parsedDate = app.parseItalianDate(form.date);
    const listino = {
        id: prezziState.editingListino ? prezziState.editingListino.id : app.generateUniqueId('listino'),
        date: parsedDate.toISOString(),
        benzina: parseFloat(form.benzina) || 0,
        gasolio: parseFloat(form.gasolio) || 0,
        dieselPlus: parseFloat(form.dieselPlus) || null,
        hvolution: parseFloat(form.hvolution) || null,
        adblue: parseFloat(form.adblue) || null
    };
...
function saveConcorrenza() {
    const app = getApp();
    const form = prezziState.concorrenzaForm;
    if (!form.date || !app.validateItalianDate(form.date)) return app.showNotification('Data obbligatoria in formato gg.mm.aaaa');
    const parsedDate = app.parseItalianDate(form.date);
    const existingIndex = app.state.data.competitorPrices.findIndex(p => p.date === parsedDate.toISOString());
    const concorrenza = {
        date: parsedDate.toISOString(),
        myoil: { benzina: parseFloat(form.myoil.benzina) || null, gasolio: parseFloat(form.myoil.gasolio) || null },
        esso: { benzina: parseFloat(form.esso.benzina) || null, gasolio: parseFloat(form.esso.gasolio) || null },
        q8: { benzina: parseFloat(form.q8.benzina) || null, gasolio: parseFloat(form.q8.gasolio) || null }
    };
    if (existingIndex !== -1) {
        app.state.data.competitorPrices[existingIndex] = { ...app.state.data.competitorPrices[existingIndex], ...concorrenza };
        app.showNotification('Prezzi concorrenza aggiornati per la data specificata.');
    } else {
        concorrenza.id = app.generateUniqueId('concorrenza');
        app.state.data.competitorPrices.push(concorrenza);
        app.showNotification('Prezzi concorrenza salvati.');
    }
    app.saveToStorage('data', app.state.data);
    app.hideFormModal();
    renderConcorrenzaCard.call(app);
...
function renderListiniTable() {
    ...
    document.querySelectorAll('#listini-table th button[data-sort]').forEach(btn => {
        ...
        if(!icon) return;
        if (btn.dataset.sort === prezziState.priceSort.column) {
            icon.setAttribute('data-lucide', prezziState.priceSort.direction === 'asc' ? 'chevron-up' : 'chevron-down');
        } else {
            icon.setAttribute('data-lucide', 'chevrons-up-down');
        }
    });
    if (listini.length === 0) {
        tbody.innerHTML = `<tr><td colspan="7" class="text-center py-12"><div class="empty-state"><i data-lucide="tag"></i><div class="empty-state-title">Nessun listino trovato</div></div></td></tr>`;
    } else {
        tbody.innerHTML = listini.map(listino => `
            <tr class="hover:bg-secondary">
                ...
                <td class="font-bold text-secondary">${listino.adblue ? app.formatCurrency(listino.adblue, true) : '-'}</td>
                <td class="text-right">
...
function renderConcorrenzaCard() {
    ...
    const formatDiff = (diff) => {
        ...
        const text = `${sign}${app.formatCurrency(roundedDiff, true)}`;
    ...
    container.innerHTML = `
        <div class="space-y-4">
            <div class="grid grid-cols-3 gap-4">
                <div class="product-box" style="background-color: #8b5cf6; border-color: #7c3aed; padding: 1rem;"><h4 class="font-semibold mb-2 text-center" style="color: #ffffff;">MyOil</h4><div class="space-y-1 mt-2"><div class="flex justify-between p-1" style="color: rgba(255, 255, 255, 0.9);"><span>Benzina</span><span class="font-bold" style="color: #ffffff;">${app.formatCurrency(competitorPricesData.myoil?.benzina || 0, true)}</span></div><div class="flex justify-between p-1" style="color: rgba(255, 255, 255, 0.9);"><span>Gasolio</span><span class="font-bold" style="color: #ffffff;">${app.formatCurrency(competitorPricesData.myoil?.gasolio || 0, true)}</span></div></div></div>
                <div class="product-box" style="background-color: #FF204E; border-color: #DC1C44; padding: 1rem;"><h4 class="font-semibold mb-2 text-center" style="color: #ffffff;">Esso</h4><div class="space-y-1 mt-2"><div class="flex justify-between p-1" style="color: rgba(255, 255, 255, 0.9);"><span>Benzina</span><span class="font-bold" style="color: #ffffff;">${app.formatCurrency(competitorPricesData.esso?.benzina || 0, true)}</span></div><div class="flex justify-between p-1" style="color: rgba(255, 255, 255, 0.9);"><span>Gasolio</span><span class="font-bold" style="color: #ffffff;">${app.formatCurrency(competitorPricesData.esso?.gasolio || 0, true)}</span></div></div></div>
                <div class="product-box" style="background-color: #06b6d4; border-color: #0891b2; padding: 1rem;"><h4 class="font-semibold mb-2 text-center" style="color: #ffffff;">Q8</h4><div class="space-y-1 mt-2"><div class="flex justify-between p-1" style="color: rgba(255, 255, 255, 0.9);"><span>Benzina</span><span class="font-bold" style="color: #ffffff;">${app.formatCurrency(competitorPricesData.q8?.benzina || 0, true)}</span></div><div class="flex justify-between p-1" style="color: rgba(255, 255, 255, 0.9);"><span>Gasolio</span><span class="font-bold" style="color: #ffffff;">${app.formatCurrency(competitorPricesData.q8?.gasolio || 0, true)}</span></div></div></div>
            </div>
            <div class="grid grid-cols-3 gap-4">
                <div class="product-box text-center p-2" style="background-color: transparent; border: 2px solid #8b5cf6;"><div>Benzina</div>${formatDiff(diffs.myoil.benzina)}<div class="mt-1">Gasolio</div>${formatDiff(diffs.myoil.gasolio)}</div>
                <div class="product-box text-center p-2" style="background-color: transparent; border: 2px solid #FF204E;"><div>Benzina</div>${formatDiff(diffs.esso.benzina)}<div class="mt-1">Gasolio</div>${formatDiff(diffs.esso.gasolio)}</div>
                <div class="product-box text-center p-2" style="background-color: transparent; border: 2px solid #06b6d4;"><div>Benzina</div>${formatDiff(diffs.q8.benzina)}<div class="mt-1">Gasolio</div>${formatDiff(diffs.q8.gasolio)}</div>
            </div>
        </div>
    `;
}

// === FUNZIONE PLACEHOLDER PER LO SKELETON LOADER ===
