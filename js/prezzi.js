...
(function() {
    ...
    const PrezziModule = {
        ...
        openListinoModal(id=null) {
            ...
            const form = `<form id="form-listino" class="space-y-4"><div class="grid grid-cols-2 gap-4"><div><label class="block text-sm font-medium mb-2 dark:text-white">Data Listino</label><input type="date" name="date" value="${i?i.date.split('T')[0]:d}" class="${cls}" required></div><div><label class="block text-sm font-medium mb-2 text-blue-600 dark:text-blue-500">AdBlue (€/L)</label><input type="number" step="0.001" name="adblue" value="${i?.adblue||''}" class="${cls}" placeholder="Opzionale"></div></div><div class="grid grid-cols-2 gap-4"><div><label class="block text-sm font-medium mb-2 text-green-600 dark:text-green-500">Benzina *</label><input type="number" step="0.001" name="benzina" value="${i?.benzina||''}" required class="${cls}"></div><div><label class="block text-sm font-medium mb-2 text-orange-600 dark:text-orange-500">Gasolio *</label><input type="number" step="0.001" name="gasolio" value="${i?.gasolio||''}" required class="${cls}"></div><div><label class="block text-sm font-medium mb-2 text-rose-600 dark:text-rose-500">Diesel+ *</label><input type="number" step="0.001" name="dieselPlus" value="${i?.dieselPlus||''}" required class="${cls}"></div><div><label class="block text-sm font-medium mb-2 text-cyan-600 dark:text-cyan-500">Hvolution *</label><input type="number" step="0.001" name="hvolution" value="${i?.hvolution||''}" required class="${cls}"></div></div></form>`;
