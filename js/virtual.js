...
function getVirtualFormHTML() {
    ...
    return `
        <div class="modal-header">
            ...
            <button type="button" id="close-virtual-icon-btn" class="modal-close-btn">
                <i data-lucide="x"></i>
            </button>
        </div>
...
function getMeseFormHTML(turno = null) {
    ...
    return `
        <div class="modal-header">
            <h2 class="card-title">${title}</h2>
            <button type="button" id="close-mese-icon-btn" class="modal-close-btn">
                <i data-lucide="x"></i>
            </button>
        </div>
        <div class="modal-body">
...
function setupVirtualFormEventListeners() {
    ...
    document.getElementById('close-virtual-icon-btn')?.addEventListener('click', () => app.hideFormModal());
    ...
    const inputs = [{
        ...
        id: 'prepay-dieselplus',
        path: 'prepay.dieselplus'
    }, {
        id: 'prepay-hvolution',
        path: 'prepay.hvolution'
    }, {
        id: 'servito-benzina',
        path: 'servito.benzina'
    }, {
        id: 'servito-gasolio',
        path: 'servito.gasolio'
    }, {
        id: 'servito-dieselplus',
        path: 'servito.dieselplus'
    }, {
        id: 'servito-hvolution',
        path: 'servito.hvolution'
    }, {
        id: 'servito-adblue',
        path: 'servito.adblue'
    }];
...
function setupMeseFormEventListeners() {
    ...
    document.getElementById('close-mese-icon-btn')?.addEventListener('click', () => app.hideFormModal());
}
...
function saveTurno() {
    ...
    } else app.state.data.turni.push(turno);
...
function saveMese() {
    ...
    const saveAction = () => {
        if (isEdit) {
            const updatedTurno = { ...virtualState.editingTurno,
                ...turnoData
            };
            const index = app.state.data.turni.findIndex(t => t.id === updatedTurno.id);
            if (index > -1) app.state.data.turni[index] = updatedTurno;
            app.showNotification('Riepilogo mensile aggiornato!');
        } else {
            const newTurno = {
            ...
            app.showNotification('Riepilogo mensile salvato!');
        }
        app.saveToStorage('data', app.state.data);
        app.hideFormModal();
        virtualState.editingTurno = null;
    ...
    if (!isEdit && existingTurno) app.showConfirm('Un riepilogo per questo mese esiste già. Vuoi sovrascriverlo?', saveAction);
    else saveAction();
}
...
function sortVirtual(column) {
    const app = this;
    if (virtualState.virtualSort.column === column) virtualState.virtualSort.direction = virtualState.virtualSort.direction === 'asc' ? 'desc' : 'asc';
    else {
...
function sortedTurni() {
    ...
    const turni = filtered.map(t => ({ ...t,
        total: getTurnoTotal.call(app, t)
    }));
    return turni.sort((a, b) => {
        ...
        } else return (a.total - b.total) * dir;
    });
}
...
function virtualStats() {
    ...
    if (filteredTurni.length === 0) return {
        totalLiters: 0,
        revenue: 0,
        servitoPercentage: 0
    };
    ...
    const prezzo_prepay_base = 0.005,
        prezzo_servito_base = 0.015,
        maggiorazione_servito = 0.210;
...
function currentPrices() {
    const app = this;
    if (!Array.isArray(app.state.data.priceHistory) || app.state.data.priceHistory.length === 0) return {
        benzina: 0,
        gasolio: 0,
        dieselPlus: 0,
        hvolution: 0,
...
function renderTurniTable() {
    ...
    if (turni.length === 0) {
        ...
        tbody.innerHTML = turni.map(turno => {
            ...
            let actionsHTML = isRiepilogo ?
                `<div class="flex items-center justify-end space-x-2"><button class="btn btn-success" onclick="editTurnoById('${turno.id}')" title="Modifica riepilogo"><i data-lucide="edit"></i></button><button class="btn btn-danger" onclick="deleteTurnoById('${turno.id}')" title="Elimina riepilogo"><i data-lucide="trash-2"></i></button></div>` :
                `<div class="flex items-center justify-end space-x-2"><button class="btn btn-success" onclick="editTurnoById('${turno.id}')" title="Modifica turno"><i data-lucide="edit"></i></button><button class="btn btn-danger" onclick="deleteTurnoById('${turno.id}')" title="Elimina turno"><i data-lucide="trash-2"></i></button></div>`;
            return `
                <tr class="hover:bg-secondary">
                    <td class="font-medium text-primary">${app.formatDate(turno.date)}</td>
                    <td>${turno.turno}</td>
                    <td>${formatVirtualProductColumn.call(app, turno, 'benzina')}</td>
                    <td>${formatVirtualProductColumn.call(app, turno, 'gasolio')}</td>
                    <td>${formatVirtualProductColumn.call(app, turno, 'dieselplus')}</td>
                    <td>${formatVirtualProductColumn.call(app, turno, 'hvolution')}</td>
                    <td>${formatVirtualProductColumn.call(app, turno, 'adblue')}</td>
                    <td class="font-bold">${app.formatInteger(turno.total)}</td>
...
function formatVirtualProductColumn(turno, product) {
    ...
    if (turno.turno === 'Riepilogo Mensile') {
        ...
        if (product === 'adblue') return `<div class="text-sm">SERV: ${app.formatInteger(serv)}</div>`;
        return `<div class="text-sm">FDT: ${app.formatInteger(fdt)}</div><div class="text-sm">SERV: ${app.formatInteger(serv)}</div><div class="text-sm">PAY: ${app.formatInteger(pay)}</div>`;
    }
    ...
    const parts = [];
    if (prepay > 0) parts.push(`<div class="text-sm">PAY: ${app.formatInteger(prepay)}</div>`);
    if (servito > 0) parts.push(`<div class="text-sm">SERV: ${app.formatInteger(servito)}</div>`);
    ...
    const centerTextPlugin = {
        ...
        beforeDatasetsDraw(chart) {
            if (chart.config.options.plugins.centerText?.display) {
                ...
                // --- INIZIO MODIFICA ---
                const isDark = app.state.isDarkMode || document.documentElement.getAttribute('data-theme') === 'night';
...
function getProductsChartData() {
    ...
    filteredTurni.forEach(turno => {
        Object.keys(totals).forEach(product => {
            if (turno.turno === 'Riepilogo Mensile') totals[product] += (parseFloat(turno.fdt?.[product]) || 0) + (parseFloat(turno.prepay?.[product]) || 0) + (parseFloat(turno.servito?.[product]) || 0);
            else if (product === 'adblue') totals[product] += parseFloat(turno.servito?.[product]) || 0;
...
function getServiceChartData() {
    ...
    for (const turno of filteredTurni) {
        for (const product of products) {
            ...
            let prepayValue = 0;
            if (product !== 'adblue') {
                if (turno.turno === 'Riepilogo Mensile') prepayValue = (parseFloat(turno.fdt?.[product]) || 0) + (parseFloat(turno.prepay?.[product]) || 0);
                else prepayValue = parseFloat(turno.prepay?.[product]) || 0;
            }
    ...
    return {
        labels: ['Benzina', 'Gasolio', 'Diesel+', 'Hvolution', 'AdBlue'],
        datasets: [{
            label: 'Prepay/FaiDaTe',
            data: Object.values(prepayTotals)
        }, {
            label: 'Servito',
            data: Object.values(servitoTotals)
        }]
    };
}
...
function getMonthlyTrendChartData() {
    ...
    yearTurni.forEach(turno => {
        ...
        let totalLiters = 0;
        if (tab === 'generale') totalLiters = getTurnoTotal.call(app, turno);
        else if (turno.turno === 'Riepilogo Mensile') totalLiters = (parseFloat(turno.fdt?.[tab]) || 0) + (parseFloat(turno.prepay?.[tab]) || 0) + (parseFloat(turno.servito?.[tab]) || 0);
        else totalLiters = (parseFloat(turno.prepay?.[tab]) || 0) + (parseFloat(turno.servito?.[tab]) || 0);
...
function updateChartsTheme() {
    // --- INIZIO MODIFICA ---
    const isDark = document.body.classList.contains('theme-dark') || document.documentElement.getAttribute('data-theme') === 'night';
    // --- FINE MODIFICA ---
...
function onVirtualSectionOpen() {
    const app = getApp();
    if (!app || !app.state || !app.state.data || !Array.isArray(app.state.data.turni)) {
        console.log('[VIRTUAL] Dati non ancora caricati...');
        setTimeout(() => {
...
function diagnosticaERiparaTurni() {
    console.log('[VIRTUAL] Inizio diagnostica turni...');
    const app = getApp();
    if (!app || !app.state.data.turni) {
        console.log('[VIRTUAL] Nessun dato turni');
        return false;
    }
    ...
    if (JSON.stringify(turniOriginali) === JSON.stringify(turniNormalizzati)) {
        console.log('[VIRTUAL] Turni normalizzati');
        return true;
    }
    console.log('[VIRTUAL] Correzione turni...');
...
function showSkeletonLoader(container) {
    const skeletonHTML = `
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-2 gap-6">
                <div class="card">
                    <div class="card-body"><div class="skeleton-loader" style="height: 300px; width: 100%;"></div></div>
                </div>
                <div class="card">
                    <div class="card-body"><div class="skeleton-loader" style="height: 300px; width: 100%;"></div></div>
                </div>
...
function editTurnoByIdRouter(id) {
    ...
    if (!turno) return;
    if (turno.turno === 'Riepilogo Mensile') showEditMeseModal(turno);
    else showEditTurno(turno);
}
