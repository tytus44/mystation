    ...
    const AppsModule = {
        ...
        saveLayout() {
            try {
                ...
                localStorage.setItem('Polaris_apps_layout_v1', JSON.stringify(layout));
            } catch (e) { console.warn('Salvataggio layout app bloccato:', e); }
        },

        restoreLayout() {
            try {
                const saved = localStorage.getItem('Polaris_apps_layout_v1');
                if (!saved) return;
        ...
        saveNotes() { try { const notes = document.getElementById('app-notes-textarea').value; localStorage.setItem('Polaris_apps_notes_v1', notes); } catch (e) { console.warn('Salvataggio note bloccato:', e); } },
        loadNotes() { try { const notes = localStorage.getItem('Polaris_apps_notes_v1'); const textarea = document.getElementById('app-notes-textarea'); if (textarea && notes) { textarea.value = notes; } } catch (e) { console.warn('Caricamento note fallito:', e); } },
