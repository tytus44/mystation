...
const CaricoModule = {
    ...
    init: function() {
        ...
        // Listener globale per chiudere il datepicker cliccando fuori
    ...
    render: function() {
        ...
        container.innerHTML = `
            ...
            <div class="dashboard-grid" style="grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); margin-bottom: 24px;">
                <div class="card">
                    <div class="card-header">
                        <span class="card-title">Totale Litri ${new Date().getFullYear()}</span>
                ...
                <div class="card">
                    <div class="card-header">
                        <span class="card-title">Top Prodotto</span>
                        <i data-lucide="tag"></i>
                    </div>
                    <div class="card-body">
                ...
                <div class="card">
                    <div class="card-header">
                        <span class="card-title">Autista Top</span>
            ...
            <div class="card" style="margin-bottom: 24px;">
                <div class="card-header">
                    <span class="card-title">Dettaglio Annuale</span>
                        ...
                        <table class="table-prices">
                            <thead>
                                <tr>
                                    <th>Prodotto</th>
                                    <th>Carico Tot.</th>
                                    <th class="text-success">Diff (+)</th>
                                    <th class="text-danger">Diff (-)</th>
                                    <th>Giacenza Anno Prec.</th>
                                    <th>Chiusura Contabile</th>
                                </tr>
                            </thead>
                            <tbody>
                                ${this.renderSummaryRows(summary)}
                            </tbody>
                            <tfoot>
                                <tr style="font-weight: 700; background-color: var(--bg-app);">
                                    <td style="padding: 12px;">TOTALE</td>
                                    <td style="padding: 12px;">${summary.total.carico.toLocaleString()}</td>
                                    <td style="padding: 12px;" class="text-success">${summary.total.pos.toLocaleString()}</td>
                                    <td style="padding: 12px;" class="text-danger">${summary.total.neg.toLocaleString()}</td>
                                    <td style="padding: 12px;">-</td>
                                    <td style="padding: 12px;">${summary.total.chiusura.toLocaleString()}</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
            ...
            <div class="card">
                <div class="card-header">
                    <span class="card-title">Registro Carichi</span>
    ...
    getPrevStock: function() {
        try { return JSON.parse(localStorage.getItem('polaris_registro_stock_prev') || '{}'); } catch (e) { return {}; }
    },
    ...
    calculateTopStats: function(entries) {
        ...
        yearEntries.forEach(e => {
            if (e.driver) {
                driverCounts[e.driver] = (driverCounts[e.driver] || 0) + 1;
            }
            ...
            this.products.forEach(p => {
                ...
                if (qty > 0) {
                    prodCounts[pid] = (prodCounts[pid] || 0) + qty;
                }
            });
        });
        ...
        if (topProdId) {
            ...
            if (prodObj) {
                topProductLabel = prodObj.label;
                topProductColor = prodObj.color;
            }
        }
    ...
    calculateAnnualSummary: function(entries) {
        ...
        yearEntries.forEach(e => {
            this.products.forEach(p => {
                ...
                if (diff > 0) summary[pid].pos += diff;
                else summary[pid].neg += diff;
            });
        });
        ...
        this.products.forEach(p => {
            ...
            summary.total.carico += s.carico;
            summary.total.pos += s.pos;
            summary.total.neg += s.neg;
            summary.total.chiusura += s.chiusura;
        });
        ...
        return this.products.map(p => {
            ...
            return `
                <tr style="border-bottom: 1px dashed var(--border-color);">
                    <td style="padding: 12px; font-weight: 600; color: ${p.color};">${p.label}</td>
                    <td style="padding: 12px;">${s.carico.toLocaleString()}</td>
                    <td style="padding: 12px;" class="text-success">${s.pos > 0 ? '+' + s.pos : '-'}</td>
                    <td style="padding: 12px;" class="text-danger">${s.neg < 0 ? s.neg : '-'}</td>
                    <td style="padding: 12px;">
                        <input type="number" class="nav-link inp-prev-year" 
                               data-prod="${pid}" value="${stockVal}" 
                               style="width: 100px; border: 1px solid var(--border-color); padding: 5px; border-radius: 8px;" 
                               placeholder="0">
                    </td>
                    <td style="padding: 12px; font-weight: bold;">${s.chiusura.toLocaleString()}</td>
                </tr>
            `;
        }).join('');
    },
            ...
            <table class="table-prices">
                <thead>
                    <tr>
                        <th>Data</th>
                        <th>Autista</th>
                        <th class="text-benzina">Benzina</th>
                        <th class="text-gasolio">Gasolio</th>
                        <th class="text-dieselplus">Diesel+</th>
                        <th class="text-hvolution">Hvolution</th>
                        <th>Azioni</th>
                    </tr>
                </thead>
                <tbody>
        ...
        pageItems.forEach(e => {
            ...
            html += `
                <tr style="border-bottom: 1px dashed var(--border-color);">
                    ...
                    <td style="padding: 12px; color: var(--text-secondary); font-size: 0.9em;">${e.driver || '-'}</td>
                    <td style="padding: 12px;">${fmtCell(e.benzina)}</td>
                    <td style="padding: 12px;">${fmtCell(e.gasolio)}</td>
                    <td style="padding: 12px;">${fmtCell(e.dieselplus)}</td>
                    <td style="padding: 12px;">${fmtCell(e.hvolution)}</td>
                    <td style="padding: 12px; text-align: right;">
                        <button class="icon-btn btn-edit" onclick="CaricoModule.openModalCarico('${e.id}')" title="Modifica"><i data-lucide="pencil" style="width:16px;"></i></button>
                        <button class="icon-btn btn-delete" onclick="CaricoModule.deleteEntry('${e.id}')" title="Elimina"><i data-lucide="trash-2" style="width:16px;"></i></button>
                    </td>
                </tr>
            `;
        });
    ...
    openModalCarico: function(idToEdit = null) {
        ...
        let driverVal = ex ? (ex.driver || ex.autistaName || '') : '';
                    ...
                    ${this.products.map(p => `
                        <div class="input-row" data-prod="${p.id}" style="display: grid; grid-template-columns: 1fr 1.5fr 1fr; gap: 15px; margin-bottom: 15px; align-items: center;">
                            ...
                            <div class="input-stepper">
                                <button type="button" class="step-btn minus" data-target="qty-${p.id}" data-step="1000"><i data-lucide="minus" style="width:14px;"></i></button>
                                <input type="number" id="qty-${p.id}" class="stepper-input inp-qty" placeholder="0">
                                <button type="button" class="step-btn plus" data-target="qty-${p.id}" data-step="1000"><i data-lucide="plus" style="width:14px;"></i></button>
                            </div>
                            
                            <div class="input-stepper">
                                <button type="button" class="step-btn minus" data-target="diff-${p.id}" data-step="1"><i data-lucide="minus" style="width:14px;"></i></button>
                                <input type="number" id="diff-${p.id}" class="stepper-input inp-diff" placeholder="0">
                                <button type="button" class="step-btn plus" data-target="diff-${p.id}" data-step="1"><i data-lucide="plus" style="width:14px;"></i></button>
                            </div>
                        </div>
                    `).join('')}
                </div>
        ...
        if (ex) {
            this.products.forEach(p => {
                const pid = p.id;
                // Nota: Recupera gli input tramite ID univoco
                const data = ex[pid] || { carico: 0, diff: 0 };
                const qtyInp = document.getElementById(`qty-${pid}`);
                const diffInp = document.getElementById(`diff-${pid}`);
                
                if(qtyInp) qtyInp.value = data.carico !== 0 ? data.carico : '';
                if(diffInp) diffInp.value = data.diff !== 0 ? data.diff : '';
            });
        }
    ...
    // NUOVA FUNZIONE: Logica stepper per carico e differenze
    setupStepper: function() {
        document.querySelectorAll('.step-btn').forEach(btn => {
            // Clone per evitare eventi duplicati se si riapre
            ...
            newBtn.addEventListener('click', (e) => {
                ...
                if(input) {
                    ...
                    if (isPlus) {
                        ...
                        val -= step;
                        // Per carico niente negativi, per differenza ok
                        if (input.classList.contains('inp-qty')) val = Math.max(0, val);
                    }
                    input.value = val;
    ...
    renderCalendar: function(year, month) {
        ...
        let html = `<div class="datepicker-header"><button type="button" class="datepicker-nav" onclick="CaricoModule.changeMonth(${month-1}, ${year}); event.stopPropagation();"><i data-lucide="chevron-left" style="width:16px;"></i></button><div class="datepicker-title">${months[month]} ${year}</div><button type="button" class="datepicker-nav" onclick="CaricoModule.changeMonth(${month+1}, ${year}); event.stopPropagation();"><i data-lucide="chevron-right" style="width:16px;"></i></button></div><div class="datepicker-grid">${days.map(d=>`<div class="datepicker-day-label">${d}</div>`).join('')}`;
        for(let i=0; i<adjFirst; i++) html+=`<div class="datepicker-day empty"></div>`;
        const selD = new Date(document.getElementById('inp-date').value);
        const today = new Date();
        for(let i=1; i<=daysInM; i++) {
            let cls = 'datepicker-day';
            if(i===today.getDate() && month===today.getMonth() && year===today.getFullYear()) cls+=' today';
            if(i===selD.getDate() && month===selD.getMonth() && year===selD.getFullYear()) cls+=' selected';
            html+=`<div class="${cls}" onclick="CaricoModule.selectDate(${year},${month},${i}); event.stopPropagation();">${i}</div>`;
        }
        html+='</div>'; w.innerHTML = html; lucide.createIcons();
    },
    ...
    saveEntry: function() {
        ...
        this.products.forEach(p => {
            const pid = p.id;
            // Recupera valori tramite ID specifico
            const qty = parseFloat(document.getElementById(`qty-${pid}`).value) || 0;
            const diff = parseFloat(document.getElementById(`diff-${pid}`).value) || 0;
        ...
        if (this.editingId) {
            ...
            showNotification("Carico aggiornato", 'success');
        } else {
            entries.push(entryData);
            showNotification("Carico salvato", 'success');
        }
    ...
    deleteEntry: function(id) {
        // MODALE CONFERMA
        const bodyHTML = `<div style="text-align:center; padding:10px;"><i data-lucide="trash-2" style="width:48px; height:48px; color:var(--col-destructive); margin-bottom:10px;"></i><p style="font-weight:600; color:var(--text-main);">Eliminare questo carico?</p></div>`;
        ...
        setTimeout(() => {
            ...
            document.getElementById('btn-confirm-del').onclick = () => {
                ...
                localStorage.setItem('polaris_registro_carico', JSON.stringify(entries));
                showNotification("Carico eliminato", 'info');
    ...
    importData: function(e) { const f = e.target.files[0]; if (!f) return; const r = new FileReader(); r.onload = (ev) => { try { const json = JSON.parse(ev.target.result); 
        // MODALE IMPORT
        ...
        setTimeout(() => {
            ...
            document.getElementById('btn-confirm-imp').onclick = () => {
                ...
                localStorage.setItem('polaris_registro_carico', JSON.stringify(entries));
                localStorage.setItem('polaris_registro_stock_prev', JSON.stringify(stock));
                showNotification("Importazione riuscita", 'success');
                this.closeModal(); this.render(); e.target.value='';
            };
        }, 0);
    } catch (err) { showNotification("File non valido", 'error'); } }; r.readAsText(f); },
